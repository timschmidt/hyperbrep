...
- closed multi-edge rational Bézier/NURBS planar boundaries decompose into
...
- degree-one `2×2` affine rational Bézier and NURBS tensor images with
  nonconstant positive rank-one weights pull rational Bézier boundaries back
  through two exact Möbius inverses and one homogeneous Bernstein denominator
  product; NURBS results retain arbitrary authored active knot domains,
  complete parameterization validation rejects interior-control forgeries,
  and higher-degree projective inverses remain explicitly
  algebraic/unsupported;
...
- exact coaxial torus/torus carrier intersection by reducing both ring-torus
  meridians to native profile circles, distinguishing coincidence, separation,
  tangency, and two latitude circles without sampling. Matching and mirrored
...
- partial coplanar `SurfaceRegion` relations transfer a line-bounded planar
  ...
  fitting or planar replacement is used. When one boundary support crosses
  tensor-face holes, its disjoint material fragments author one exact
  boundary-component bridge cycle. Crossed components compact into the two
  descendant outer wires, surplus wire slots are retired through an explicit
  old-to-new ID map, and every bridge edge remains identity-shared.
