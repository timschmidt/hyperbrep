...
| `revolve_path` / `revolve_path_region` | Four periodic cells per exact `CurvePath2` carrier; retained Bézier/B-spline/NURBS meridians and curved cavity shells | Line/arc faces exact; general spline faces explicitly unsupported | Exact for polynomial-equivalent boundary moments; genuinely rational unimplemented moments explicitly unsupported | Exact radial/curved-path test | Rigid / exact |
